...
function print_match($match, $id_user, $final_name, $previous_final_name){		
	...
	if(sizeof($paris)>=1){    // Il existe un pari: on l'imprime
		...
		if($phase<=1){
		  ...
		  echo "<input type='hidden' name='modify_bet_phase' value='$final_name'>";
		...
		if($phase>1)
			...
			if($done==3){ // Si done == 3, on ne connait pas encore les équipes du match!
				...
				echo get_name($id_team_A);
				print_form_score();
				echo get_name($id_team_B);
			}			
...
if(isset($_POST['send_bet'])){
	...
	if($pari_A==$pari_B) header("Location:paris_finales.php?erreur=even#top"); // Erreur s'il y a égalité
		...
		$q = "SELECT id_pari FROM ".get_table_paris()." WHERE id_match=$id_match AND id_user=$id_user";
		$r = mysql_query($q) or die(mysql_error());
		if(mysql_num_rows($r)==0){  // Faire bien gaffe à ne pas enregistrer 2 paris pour le même match !
			$q = "INSERT INTO ".get_table_paris()."(id_match,id_user,pari_A,pari_B,id_team_A,id_team_B,win,penalties)
...
if(isset($_POST['modify_bet'])){
	...
	$test=0;
	if ($phase != 'finale'){
		$q = "SELECT id_match FROM ".get_table_paris()." WHERE id_user=$id_user"; // On ne peut pas annuler un pari si des paris dépendent de son issue
		...
		while($pari = mysql_fetch_array($r_paris)){
			...
			$q = "SELECT id_match FROM ".get_table_matchs()." WHERE id_match=$id_match AND (";
			if($phase=='8emes')
	...
	if ($test==0){
		$q = "DELETE FROM ".get_table_paris()." WHERE id_pari=$id_pari";
		mysql_query($q) or die(mysql_error());
		header("Location:paris_finales.php#$phase");
	}else
...
<h1>Phase finale</h1>
<h3>(modifiable jusqu'au 29/06 18:00)</h3>
...
<?php
  ...
  foreach(get_matchs_8emes() as $match){
	...
	print_match($match, $id_user, '8emes', '');
	echo "</div>";
  }
?>
...
<?php
  foreach(get_matchs_4rts() as $match){
	...
	print_match($match, $id_user, '4rts', '8emes');
	echo "</div>";
  }
?>
...
<?php
  foreach(get_matchs_demis() as $match){
	echo "<div class=\"col-xs-6 margin_down bet\">";
	print_match($match, $id_user, 'demis', '4rts');
	echo "</div>";
  }
?>
...
<?php
  $matchs = get_match_finale();
  print_match($matchs[0], $id_user, 'finale', 'demis');
?>
</div>
</div>
