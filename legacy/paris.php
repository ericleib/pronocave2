...
if(isset($_POST['modify_poule_bet'])){  // On a cliqué sur modfier
	  if(get_phase()==$PHASE_PRONO){       // Si on est en phase de paris
		    $poule   = $_POST['modify_poule_bet'];
		    for($i=0; $i<=5; $i++)  $id_pari[$i] = $_POST['id_pari_'.$i]; // Récupérer les paris modifiés
		    $q = "DELETE FROM ".get_table_paris()." WHERE id_pari='$id_pari[0]'
...
if(isset($_POST['set_poule_bet'])){    // On a cliqué sur "OK"
	if(get_phase()==$PHASE_PRONO){     // Si on est bien en phase "paris poule"
		$id_user = $_SESSION['current_ID'];
		$id_match= $_POST['id_match_0'];
		$poule   = $_POST['set_poule_bet'];
		$q= "SELECT id_match FROM ".get_table_paris()." WHERE id_match=$id_match AND id_user=$id_user"; //On sélection le premier match modifié
		...
		if(mysql_num_rows($r)==0){      // Si il n'y avait pas déjà de paris pour ce match...
			for($i=0; $i<=5; $i++){
				...
				elseif($pari_A>$pari_B)
				    ...
				    $win='even';
				$q = "INSERT INTO ".get_table_paris()."(id_match,id_user,pari_A,pari_B,win)
...
<h3>(modifiable jusqu'au 14/06 21:00)</h3>

<?php
if(isset($_GET['log_as']) && get_phase()==$PHASE_GROUP){
...
<tbody>
<?php
$q = "SELECT * FROM ".get_table_matchs()." WHERE phase='poules' ORDER BY poule ASC, date ASC";
...
while ($match = mysql_fetch_array($r_matchs)){  // Pour chaque match
    ...
    if($cpt_poule==0){
      ...
      $q= "SELECT id_pari FROM ".get_table_paris()." WHERE id_user=$id_user AND id_match=$id_match";
    ...
    if($n_pari){    // Il y a un paris pour ce match (test du premier match de la poule)
		$q= "SELECT * FROM ".get_table_paris()." WHERE id_user=$id_user AND id_match=$id_match";
		$r_pari = mysql_query($q) or die(mysql_error());
    ...
    if($cpt_poule==5){ // 5 = 0.5*N*(N-1)-1 avec N= nombre d'équipes/poule
		...
		$disabled = get_phase()==$PHASE_PRONO? '' : ' disabled';
	    echo "<td rowspan='6' style='vertical-align: middle;'>
	          <input type='hidden' name='$php_post' value=$poule>
	          <input type='submit' class='btn btn-default $disabled' value='$text_button'></td>";
    }
    echo "</tr>";
    $cpt_poule++;
}
