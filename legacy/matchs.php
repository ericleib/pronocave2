...
$q = "SELECT * FROM ".get_table_matchs()." WHERE done=1 OR done=5 ORDER BY date DESC";
...
while($match = mysql_fetch_array($r_matchs)){   // Pour chaque match terminé
    ...
    $cote_B=0;
    $points=0;
    $q = "SELECT * FROM ".get_table_paris()." WHERE id_match=$id_match";
...
$q = "SELECT * FROM ".get_table_matchs().",".get_table_paris()." WHERE ".get_table_matchs().".id_match=".get_table_paris().".id_match AND ".get_table_matchs().".phase='finale'";
...
<?php
$id_user = $_SESSION['current_ID'];
$q = "SELECT * FROM ".get_table_matchs()." WHERE done=0 OR done=4 ORDER BY date ASC";
$r_matchs = mysql_query($q) or die(mysql_error());
while($match = mysql_fetch_array($r_matchs)){
    ...
    if ($done==0)   // Phase de poule
    	$q = "SELECT win,pari_A,pari_B,id_user,penalties FROM ".get_table_paris()." WHERE id_match=$id_match";
